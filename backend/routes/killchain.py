...
async def compute_kill_chain(db: Session = Depends(get_db)):
    ...
    for acc in compromised:
        ...
        add_node(
            node_id,
            "COMPROMISED_ACCOUNT",
            f"{acc.account_id} ({acc.name.split()[0]} {acc.name.split()[-1][0]}.)",
            int(acc.cyber_score),
            {"status": acc.status}
        )
        edges.append({
    ...
    mules = db.query(Account).filter(
        Account.status.in_(['FLAGGED', 'UNDER_REVIEW']),
    ...
    flagged_txns = db.query(Transaction).filter(
        Transaction.status.in_(['FLAGGED', 'SUSPICIOUS'])
    ...
    for txn in flagged_txns:
        ...
        matching_mule = next(
            (m for m in mules if m.account_id == txn.account_id),
            None
        )
        if matching_mule:
            edges.append({
    ...
    suspicious_logins = db.query(LoginEvent).filter(
        LoginEvent.risk_score > 75
    ).order_by(LoginEvent.risk_score.desc()).limit(3).all()

    for login in suspicious_logins:
        login_node_id = f"login_{login.id}"
        label = f"Login: {login.country_code or 'Unknown'}"
